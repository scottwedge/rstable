...
def isstaff(checkedid, guildroles, authorroles):
...
def formatfromk(amount, currency):
    if amount >= 1000000:
        if len(str(amount)) == 7:
            return '{0:.3g}'.format(amount * 1e-06) + 'B'
        elif len(str(amount)) == 8:
            return '{0:.4g}'.format(amount * 1e-06) + 'B'
        else:
            return '{0:.5g}'.format(amount * 1e-06) + 'B'
    elif amount >= 1000:
        if len(str(amount)) == 4:
...
def printbj(user, stood, description, color):
    ...
    if stood:
        ...
        size = 1
        bot += str(get(client.emojis, name='cardback'))
...
async def on_message(message):
    ...
    ###############################
        ...
        if isstaff(message.author.id, message.guild.roles, message.author.roles) == 'verified':
    ...
    ###############################
        ...
        if str(message.channel.id) == 585098613152153600:
            ...
            await message.channel.send('This command can only be used in <#585098613152153600> to prevent spam.')
    ###############################
        ...
        if (getvalue(int(member.id), 'privacy', 'rsmoney') == False) or (isstaff(message.author.id, message.guild.roles, message.author.roles) == 'verified'):
    ...
    ###############################
        ...
        try:
            ...
            await message.channel.send('An **error** occured. Make sure you use `$clear (rs3 or 07) (@user)`')
    ###############################
        ...
        try:
            ...
            await message.channel.send('An **error** has occured. Make sure you use `$update (@user) (amount) (rs3 or 07)`.')
    ###############################
    ...
    ##################################
        ...
        try:
            ...
            if str(message.content).split(' ')[3] == 'rs3':
                if transfered < 1:
                    await message.channel.send('You must transfer at least **1k** 07.')
                    enough = False
            elif str(message.content).split(' ')[3] == '07':
                if transfered < 1:
                    await message.channel.send('You must transfer at least **1k** RS3.')
                    enough = False
            ...
            await message.channel.send('An **error** has occured. Make sure you use `$transfer (@user) (Amount you want to give) (rs3 or 07)`.')
    ##################################
        ...
        try:
            ...
            await message.channel.send('An **error** has occured. Make sure you use `$(50, 53, 75, or 95) (BET) (rs3 or 07)`.')
    ##################################
    ...
    ##################################
        ...
        try:
            ...
            if isenough(bet, currency)[0]:
                if current >= bet:
                    try:
                        ...
                        tester = int(c.fetchone()[0])
                        await message.channel.send('You are already in a game of blackjack! Type `hit` or `stand` to continue the game!')
                    except:
                        ...
                        sent = await message.channel.send(embed=printbj(message.author, False, 'Use `hit` to draw or `stand` to pass.', 28))
            ...
            await message.channel.send('An **error** has occured. Make sure you use `$bj (Amount) (rs3 or 07)`.')
    ##################################
        ...
        if playerscore > 21:
            ...
            await sent.edit(embed=printbj(message.author, False, 'Use `hit` to draw or `stand` to pass.', 28))
    ##################################
        ...
        if botscore > 21:
            await sent.edit(embed=printbj(message.author, True, 'Dealer Busts. You Win!', 3407616))
            update_money(message.author.id, bet * 2, currency)
            win = True
        elif botscore == playerscore:
            await sent.edit(embed=printbj(message.author, True, 'Tie! Money Back.', 16776960))
            update_money(message.author.id, bet, currency)
        elif playerscore > botscore:
            await sent.edit(embed=printbj(message.author, True, "Your score is higher than the dealer's. You Win!", 3407616))
            update_money(message.author.id, bet * 2, currency)
            win = True
        elif botscore > playerscore:
            await sent.edit(embed=printbj(message.author, True, "The dealer's score is higher than yours. You lose.", 16711718))
    ...
    ##################################################
        ...
        if kind == 'bronze':
            if bronze >= 1:
                ...
                await message.channel.send("You don't have any bronze keys to open!")
        elif kind == 'silver':
            if silver >= 1:
                ...
                await message.channel.send("You don't have any silver keys to open!")
        elif kind == 'gold':
            if gold >= 1:
                ...
                await message.channel.send("You don't have any gold keys to open!")
    ...
    ############################################################
        ...
        try:
            ...
            await message.channel.send('An **error** has occured. Make sure you use `$fp (Amount) (rs3 or 07)`.')
    ####################################################
